...
- **REST API（JSON）** を基本方針とする。Django REST Framework（DRF）の利用を推奨する。
  - `TODO`: DRFを新規依存として追加するか、Django標準の `JsonResponse` ベースで自作するかは、
    `01_プロジェクト概要.md` の「Django標準機能を優先」方針との整合を実装時に検討する。
    ただしAPI規模が大きくなる想定（一覧取得・状態更新・事前登録CRUD・お気に入りCRUD等）のため、DRF採用を推奨する。
- 認証: セッション認証（Djangoログインセッションをそのまま利用）。トークン認証は導入しない（社内専用のため不要）。
- IP制限: ネットワーク層（Oracle Cloudのセキュリティリスト／ファイアウォール等）で実現する前提とし、
  **Djangoアプリケーション側での二重チェックは行わない**（過剰実装を避け、保守性を優先）。
...
| GET | `/api/destinations/recent/` | 直近30日の使用履歴からの自動候補取得（`05_データモデル.md` 8章） |
...
- `07_認証・セキュリティ詳細.md` にて、ログイン・OTP・セッション管理の実装詳細を確定する。
